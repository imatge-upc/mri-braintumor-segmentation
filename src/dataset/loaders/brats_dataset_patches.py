...
from src.dataset.utils.io_patch import load_patch
...
class BratsDatasetPatches(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        flair = self._load_volume_modality(os.path.join(root_path, self.data[idx].flair), BratsDatasetPatches.flair_idx)
        t1 = self._load_volume_modality(os.path.join(root_path, self.data[idx].t1), BratsDatasetPatches.t1_idx)
        t2 = self._load_volume_modality(os.path.join(root_path, self.data[idx].t2), BratsDatasetPatches.t2_idx)
        t1_ce = self._load_volume_modality(os.path.join(root_path, self.data[idx].t1ce), BratsDatasetPatches.t1_idx)
        ...
        patch_modality = torch.from_numpy(patch_modality)
    ...
    def _load_volume_gt(self, seg_mask: str) -> np.ndarray:
        ...
        # segmentation, _ = load_patch(seg_mask, normalize=False) # segmentation --> always false
        return segmentation
